...
const ELEMENT_COLORS: Record<string, { bg: string; border: string; text: string }> = {
  金: { bg: 'bg-yellow-900/30', border: 'border-yellow-500/40', text: 'text-yellow-300' },
  木: { bg: 'bg-green-900/30',  border: 'border-green-500/40',  text: 'text-green-300'  },
  水: { bg: 'bg-blue-900/30',   border: 'border-blue-500/40',   text: 'text-blue-300'   },
  火: { bg: 'bg-red-900/30',    border: 'border-red-500/40',    text: 'text-red-300'    },
  土: { bg: 'bg-orange-900/30', border: 'border-orange-500/40', text: 'text-orange-300' },
};
...
function ScoreCircle({ score, size = 64 }: { score: number; size?: number }) {
  ...
  return (
    <div className="relative flex items-center justify-center flex-shrink-0" style={{ width: size, height: size }}>
      ...
      <span className={`font-mono font-bold text-xs sm:text-sm ${SCORE_COLOR(score)}`}>{score}</span>
    </div>
  );
}
...
function PillarBox({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <div className="bg-slate-800 border border-amber-500/20 rounded-lg px-2.5 sm:px-3 py-1.5 sm:py-2 text-center">
        <span className="text-amber-300 font-bold text-base sm:text-lg tracking-widest">{value}</span>
      </div>
    </div>
  );
}

function TimeframeCard({ title, timeframe }: { title: string; timeframe: { score: number; title: string; reading: string } }) {
  return (
    <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-3 sm:p-5 flex flex-col gap-2 sm:gap-3">
      <div className="flex items-center justify-between">
        <span className="text-[10px] sm:text-xs font-mono text-slate-400 uppercase tracking-widest">{title}</span>
        <ScoreCircle score={timeframe.score} size={44} />
      </div>
      <div>
        <h4 className={`font-bold text-xs sm:text-sm mb-1 ${SCORE_COLOR(timeframe.score)}`}>{timeframe.title}</h4>
        <p className="text-[11px] sm:text-xs text-slate-400 leading-relaxed">{timeframe.reading}</p>
      </div>
    </div>
  );
}
...
const CompatibilityPanel: React.FC<CompatibilityPanelProps> = ({ reading, language, hidePrivateInfo = false }) => {
  ...
  const stockElemLabel = isCN ? '股票五行' : 'Stock Element';
  const userElemLabel = isCN ? '你的用神' : 'Lucky Element';
  const monthLabel = isCN ? '当月运势' : 'This Month';
  const yearLabel  = isCN ? '当年运势' : 'This Year';
  const longLabel  = isCN ? '长期缘分' : 'Long-Term';
  const overallLabel = isCN ? '总体匹配' : 'Overall';

  const elemEmoji = reading.stockElement === '金' ? '🪙' : reading.stockElement === '木' ? '🌿' :
    reading.stockElement === '水' ? '💧' : reading.stockElement === '火' ? '🔥' : '🌍';

  return (
    <div className="bg-gradient-to-b from-slate-900 to-slate-950 border border-amber-500/20 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 space-y-5 sm:space-y-7 shadow-2xl shadow-amber-900/10">

      {/* Header */}
      <div className="text-center space-y-1">
        <p className="text-[10px] sm:text-xs font-mono text-amber-500/60 tracking-widest uppercase">☯ 命理股票缘分测算</p>
        <h3 className="text-base sm:text-xl font-bold text-amber-300">
          {isCN ? '八字 × 股票 匹配报告' : 'BaZi × Stock Destiny'}
        </h3>
      </div>
      ...
      <div className={`grid grid-cols-1 ${hidePrivateInfo ? '' : 'sm:grid-cols-2'} gap-3 sm:gap-4`}>
        <div className={`flex items-center gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl border ${elemColors.bg} ${elemColors.border}`}>
          ...
          <div className="min-w-0">
            <p className="text-[10px] sm:text-xs text-slate-500 font-mono">{stockElemLabel}</p>
            <p className={`font-bold text-xs sm:text-sm ${elemColors.text}`}>
              {reading.stockElement}（{reading.stockElementEn}）
            </p>
            <p className="text-[10px] sm:text-xs text-slate-500 truncate">{reading.stockElementReason}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        <TimeframeCard title={monthLabel} timeframe={monthly} />
        <TimeframeCard title={yearLabel}  timeframe={yearly} />
        <TimeframeCard title={longLabel}  timeframe={longTerm} />
      </div>

      {/* Overall Verdict */}
      <div className={`rounded-xl border p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-5 ${verdictStyle}`}>
        <div className="flex items-center gap-3 sm:gap-4">
          <ScoreCircle score={reading.overallScore ?? 0} size={60} />
          <div>
            <p className="text-[10px] sm:text-xs font-mono text-slate-500 uppercase tracking-widest mb-0.5">{overallLabel}</p>
            <p className="text-lg sm:text-xl font-bold">{reading.verdict ?? '-'}</p>
          </div>
        </div>
        <div className="sm:border-l sm:border-slate-700 sm:pl-5">
          <p className="text-xs sm:text-sm leading-relaxed text-slate-300 italic">「{reading.verdictDetail ?? ''}」</p>
        </div>
      </div>

      <p className="text-center text-[10px] sm:text-xs text-slate-700">
        {isCN ? '* 本报告仅供娱乐，不构成投资建议' : '* Entertainment only. Not financial advice.'}
      </p>
    </div>
  );
};
